...
void setup() {
  ...
  pinMode(LED_SECA, OUTPUT);
  pinMode(LED_ECO, OUTPUT);
  digitalWrite(LED_SECA, LOW);
...
void loop() {
  if (botaoApertado(BTN_TOGGLE)) {
    ...
    digitalWrite(LED_SECA, modoAtual == SECAGEM);
    digitalWrite(LED_ECO,  modoAtual == ECONOMIA);
    lcd.clear();
    switch(modoAtual) {
  ...
  switch(modoAtual) {
    case SECAGEM:
      if (!lampadaLigada && temp < T_SECA_LIGA) {
        digitalWrite(RELE_PIN, HIGH); lampadaLigada = true;
      } else if (lampadaLigada && temp >= T_SECA_DESLIGA) {
        digitalWrite(RELE_PIN, LOW);  lampadaLigada = false;
      }
      analogWrite(FAN_PIN, 180);
      break;

    case ECONOMIA:
      if (!lampadaLigada && temp < T_DORME_LIGA) {
        digitalWrite(RELE_PIN, HIGH); lampadaLigada = true;
      } else if (lampadaLigada && temp >= T_DORME_DESLIGA) {
        digitalWrite(RELE_PIN, LOW);  lampadaLigada = false;
      }
    ...
    case VENTILA:
      // lâmpada em novo range de 30–50 °C
      if (!lampadaLigada && temp < T_VENT_LIGA) {
        digitalWrite(RELE_PIN, HIGH); lampadaLigada = true;
      } else if (lampadaLigada && temp >= T_VENT_DESLIGA) {
        digitalWrite(RELE_PIN, LOW);  lampadaLigada = false;
      }
